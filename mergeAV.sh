...
# Debugging output
pd () {
    ${DEBUG:-false} && echo "[${progn}-${BASHLINENO}] $@" >&2
}
...
if [ -f /tmp/cameraInit.sh-vars.txt ] ; then
    ...
    pi "cameraInit.sh does not appear to be running. "
    pi "Please start cameraInit.sh first to initialize the video loopback device. Exiting."
    exit 1
fi

./record-camera -o "${_now}.mkv" || {
    pi "Failed to start video capture. Exiting."
    rm "${_now}.mkv" 2> /dev/null
    exit 1
}

pi "AUD_DEVICE: $AUD_DEVICE" 
...
[ -d /proc/$(cat /tmp/captureInit.sh-record.pid) ] || {
    pi "ffmpeg process did not start successfully. Exiting."
    kill -HUP $audioPid
...
[ -d /proc/$audioPid ] || {
    pi "Audio process did not start successfully. Exiting."
...
cleanup() {
    trap - TERM INT
    pi ""
    pi "Shutting down video capture recording. "
    ./record-camera -q
    sleep .1
    pi "Shutting down audio $audioPid "
    kill -0 $audioPid >& /dev/null && kill -TERM $audioPid
    sleep .5

    if [ -f "${_now}.mkv"  -a  -f "${_now}.wav" ] ; then
        pi " Merging audio and video into ${_now}-merged.mkv... "
        ffmpeg -hide_banner -loglevel error -i "${_now}.mkv" -i "${_now}.wav" -c:v copy -c:a aac "${_now}-merged.mkv"
    else 
        pi "One or both of the output files ${_now}.mkv and ${_now}.wav are missing. Skipping merge." 
    fi
    exit
}
