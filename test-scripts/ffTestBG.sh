...
sig=${sig:-"-TERM"}   # <- This seems to be the best signal for ffmpeg to gracefully stop recording and finalize the output file. 
...
main () {
    ...
    "${ffcmd[@]}" &
    ffpid=$!    
    echo "Started ffmpeg with PID $ffpid" >&2
...
main &
bgpid=$!
echo "Started main function in background with PID $bgpid" >&2
